...
import com.edureka.browsers.Chrome;
import com.edureka.browsers.IE;
import com.edureka.testcase.SeleniumAction;

/**
 * class to perform execution operations
...
public class Execution {
	...
	/**
	 * This Method get the arguments from Excel
	 * @param url, browser, filePath
	 */
	...
	/**
	 * This Method to execute functions with Verify, VALUE, BACK, OBJECTEXIST
	 * uses switch case to execute
	 * @param outcome
	 */
	public boolean run(Outcome outcome) throws Exception {
		...
		for(int index = 0; index < testScripts.size(); index++){
			...
			for(int i = 0; i < script.size(); i++){
				...
				if( object.equalsIgnoreCase( "Execute" ) ) {
					...
					else if( validator.equals("BACK") ) {
						String navigateBack = this.driver.getCurrentUrl();
						System.out.println("printing current URL value :" +navigateBack);
						if( navigateBack.equals(url ) )
							System.out.println("Pass");
						else
							System.out.println("Fail");
					}
